...
def setup(duthosts, nbrhosts, nbrhosts_facts, all_cfg_facts):
    # setup - disable BGP neighbors on dut
    ...
    for per_host in duthosts.frontend_nodes:
        for asic in per_host.asics:
            ...
            for neighbor in neighs:
                if cfg_facts['BGP_NEIGHBOR'][neighbor]['name'].startswith("VM"):
...
def test_neighbor_clear_all(duthosts, setup, nbrhosts, nbrhosts_facts, all_cfg_facts, established_arp):
    """
    ...
    * Send full mesh traffic and verify relearn and DB.
    ...
    Args:
        ...
        nbrhosts_facts: nbrhost_facts fixture from voq/conftest.py
    ...
    check_all_neighbors_present(duthosts, nbrhosts, nbrhosts_facts, all_cfg_facts)


def test_neighbor_clear_one(duthosts, setup, nbrhosts, nbrhosts_facts, all_cfg_facts, established_arp):
    """
...
def change_mac(nbr, intf, mac):
    ...
    """
    nbr['host'].eos_command(commands=["enable",
                                      "bash sudo ifconfig {} down".format(intf),
                                      "bash sudo ifconfig {} hw ether {}".format(intf, mac),
                                      "bash sudo ifconfig {} up".format(intf)])
...
def check_arptable_mac(host, neighbor, mac):
...
def test_neighbor_hw_mac_change(duthosts, enum_rand_one_per_hwsku_frontend_hostname, enum_asic_index,
                                setup, nbrhosts, nbrhosts_facts, established_arp, all_cfg_facts):
    """
    ...
    * Verify that packets can be sent from local and remote linecards to the learned address.
    ...
    for neighbor in neighs:
        ...
        nbrinfo = get_neighbor_info(neighbor, nbrhosts, nbrhosts_facts)
        logger.info("*" * 60)
...
class TestNeighborLinkFlap(object):
    ...
    def test_front_panel_admindown_port(self, duthosts, enum_rand_one_per_hwsku_frontend_hostname, enum_asic_index, all_cfg_facts,
                                        setup, nbrhosts, nbrhosts_facts, established_arp):
        """
        ...
        * Admin interface up, verify recreation after restarting traffic.
        ...
        intfs, intfs_to_test = self.pick_ports(cfg_facts)
...
class TestGratArp(object):
    ...
    def test_gratarp_macchange(self, duthosts, enum_rand_one_per_hwsku_frontend_hostname, enum_asic_index,
                               ptfadapter, tbinfo, nbrhosts, nbrhosts_facts,
        ...
        * Verify that packets can be sent from local and remote linecards to learned address.
        ...
        Args:
            ...
            nbrhosts_facts: The nbrhosts_facts fixture from voq/conftest.py
        ...
        for neighbor in neighs:
            ...
            check_one_neighbor_present(duthosts, duthost, asic, neighbor, nbrhosts, nbrhosts_facts,
                                       all_cfg_facts)

            try:
                change_mac(nbrhosts[nbrinfo['vm']], nbrinfo['shell_intf'], NEW_MAC)
                self.send_grat_pkt(NEW_MAC, neighbor, int(tb_port))
                pytest_assert(wait_until(60, 2, check_arptable_mac, duthost, neighbor, NEW_MAC),
                              "MAC {} didn't change in ARP table".format(NEW_MAC))
                sonic_ping(asic, neighbor)
                check_one_neighbor_present(duthosts, duthost, asic, neighbor, nbrhosts, nbrhosts_facts,
                                           all_cfg_facts)
