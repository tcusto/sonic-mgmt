...
from tests.common.helpers.redis import AsicDbCli, AppDbCli, VoqDbCli, RedisNoCommandOutput
...
def check_host_arp_table_deleted(host, neighbor_ip):
    """
    ...
    if neighbor_ip in table:
        ...
        logger.info("IP %s is not in %s ARP table" % (neighbor_ip, host.hostname))
...
def get_neighbor_info(neigh_ip, nbrhosts, nbrhosts_facts):
    """
    ...
    Args:
        ...
        nbrhosts_facts: dictionary provided by the nbrhosts_facts fixture in voq/conftest.py

    Returns:
        A dictionary with mac, vmname and port name.
    """
    nbr_vm = ""
    nbr_intf = ""
    ...
    logger.info("Neighbor VM: %s, PC: %s, mac: %s", nbr_vm, nbr_intf, mac)
    return {'mac': mac, "port": nbr_intf, "shell_intf": shell_intf, "vm": nbr_vm}
...
def check_one_neighbor_present(duthosts, per_host, asic, neighbor, nbrhosts, nbrhosts_facts, all_cfg_facts):
    """
    ...
    Args:
        ...
        nbrhosts_facts: The neighbor facts fixture from voq/conftest.py.
    ...
    neigh_mac = get_neighbor_info(neighbor, nbrhosts, nbrhosts_facts)['mac']
...
def check_all_neighbors_present(duthosts, nbrhosts, nbrhosts_facts, all_cfg_facts):
    """
    ...
    for per_host in duthosts.frontend_nodes:
        ...
        for asic in per_host.asics:
            ...
            for neighbor in neighs:
                check_one_neighbor_present(duthosts, per_host, asic, neighbor, nbrhosts, nbrhosts_facts, all_cfg_facts)


def asic_cmd(asic, cmd):
    """
    ...
    if asic.namespace is not None:
        ...
        return asic.sonichost.command(fullcmd)
    else:
        return asic.sonichost.command(cmd)


def sonic_ping(asic, ipaddr, count=2, timeout=3, interface=None, size=None, ttl=None, verbose=False):
    """
...
def check_neighbor_is_gone(duthosts, all_cfg_facts, per_host, asic, neighbor):
    """
    ...
    # check local host
    check_host_arp_table_deleted(per_host, neighbor)
    with pytest.raises(RedisNoCommandOutput):
        asicdb.get_neighbor_key_by_ip(neighbor)
    with pytest.raises(RedisNoCommandOutput):
        appdb.get_neighbor_key_by_ip(neighbor)

    # check supervisor
    for sup in duthosts.supervisor_nodes:
        voqdb = VoqDbCli(sup)
        with pytest.raises(RedisNoCommandOutput):
            voqdb.get_neighbor_key_by_ip(neighbor)
    ...
    for rem_host in duthosts.frontend_nodes:
        ...
        for rem_asic in rem_host.asics:
            ...
            with pytest.raises(RedisNoCommandOutput):
                appdb.get_neighbor_key_by_ip(neighbor)
            check_host_arp_table_deleted(rem_host, neighbor)
            rem_cfg_facts = all_cfg_facts[rem_host.hostname][rem_asic.asic_index]['ansible_facts']
            remote_inband_info = get_inband_info(rem_cfg_facts)
            check_neighbor_kernel_route(per_host, rem_asic.asic_index, neighbor, remote_inband_info['port'],
                                        present=False)
...
def eos_ping(eos, ipaddr, count=2, timeout=3, interface=None, size=None, ttl=None, verbose=False):
    ...
    cmd = "bash sudo ping -c{c} {ip} -W {to}".format(c=count, ip=ipaddr, to=timeout)
    if interface is not None:
        cmd += " -I {}".format(interface)
    if size is not None:
        cmd += " -s {}".format(size)
    ...
    output = eos.eos_command(commands=[cmd])
    if verbose:
        logger.info("Ping  : %s" % cmd)
        logger.info("Result: %s", output['stdout_lines'][0][-2:])

    output['parsed'] = parse_ping(output['stdout_lines'][0])

    # The [0] is the command index, since eos_command takes an array.
    if "error code" in output['stdout_lines'][0][-1]:
